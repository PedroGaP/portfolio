<script>
  ...
  $: technologies = ['Todos', ...new Set(projects.flatMap(p => p.technologies))];

  $: filteredProjects = selectedTech === 'Todos'
    ? projects
  ...
  <div class="max-w-6xl mx-auto px-4 {visible ? 'opacity-100' : 'opacity-0'} transition-opacity duration-1000">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {#each filteredProjects as project}
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow transform hover:-translate-y-1 duration-300">
          ...
          <div class="p-6">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
              {#each project.technologies as tech}
                <span class="flex items-center gap-1 px-3 py-1 text-sm bg-gray-100 dark:bg-gray-800 rounded-full">
                  <Icon 
                    icon={techIcons[tech]} 
